    ...
    export default{
        ...
        methods: {
            async editProduct() {
                ...
                // await axios.patch(`${server.baseURL}/produse/update-product/${this.clickedId}`, productData);

                router.push({name: 'home'})
            },
            async getProduct() {
               const data = await axios.get(`${server.baseURL}/produse/get-product/${this.id}`);
                // const data = await axios.get(`${server.baseURL}/produse/get-product/${this.clickedId}`);
